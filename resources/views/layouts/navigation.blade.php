    ...
    <div class="container mx-auto flex items-center justify-between whitespace-nowrap px-4 py-3">
        ...
        <div class="flex items-center gap-4">
            ...
            <div class="flex items-center gap-2 text-[var(--primary-color)]">
                ...
                <h2 class="text-[var(--text-primary)] text-lg font-bold leading-tight tracking-[-0.015em]">
                    FORMA</h2>
            </div>
        ...
        <div class="flex items-center gap-3">
            <!-- Search Input -->
            <label class="hidden sm:flex flex-col min-w-32 !h-9 max-w-56">
                <div class="flex w-full flex-1 items-stretch rounded-lg h-full shadow-sm">
                    ...
                    <input class="input-search rounded-r-lg pl-2 text-sm" placeholder="Search..." value="" />
                </div>
            </label>
                ...
                <nav class="flex items-center justify-end gap-2">
                    @auth
                        @if (Auth::user()->role == 'anggota')
                            <a href="{{ route('event.create') }}"
                                class="btn-primary flex min-w-[70px] max-w-[400px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-9 px-3 text-xs font-semibold leading-normal tracking-[0.015em] shadow-sm">
                                Tambah Event
                            </a>
                        ...
                        <div class="hidden sm:flex sm:items-center">
                            <x-dropdown align="right" width="48">
                                <x-slot name="trigger">
                                    <button class="inline-flex items-center px-2 py-1 text-sm leading-4 font-medium rounded-md text-gray-500 dark:text-gray-400 bg-white dark:bg-white-800 hover:text-gray-700 dark:hover:text-gray-800 focus:outline-none transition ease-in-out duration-150">
                                        <div class="bg-center bg-no-repeat px-1 aspect-square bg-cover rounded-full size-8 border-2 border-[var(--primary-color)] shadow-sm"
                                            style='background-image: url("https://lh3.googleusercontent.com/aida-public/AB6AXuDAdiz0iXaZ6d07jjE8U5mTtqAT_j071S1NNchO5yMwazmLM8Wc-VAVo6nSlOA0CUbf2UbljcIi3dopMtHNvVuDFFQFOo4PbXSxBbyRVY60suy2t-YVL2NZCRpg01mgflWKrkXAOSyk225LwzodeZ-RG_4NmK5XQGb97rkOb-515CKo8rZmBbyHrN4sa8xOzyXm7MCb4eIJiM8adelkzaoB-QVW5EJNsI0keAdGukYksGScBMyoxqG59iZA4fxehNP-IHt2OftkgaBH");'>
                                        </div>
                                        <div class="px-2 text-sm">{{ Auth::user()->name }}</div>
                                        <div class="ms-1">
                                ...
                                <x-slot name="content">
                                    {{ $content ?? '' }} <!-- Menambahkan penanganan untuk variabel $content -->
                                    <x-dropdown-link :href="route('profile.edit')">
                                        {{ __('Profile') }}
                                    </x-dropdown-link>
                                    <form method="POST" action="{{ route('logout') }}">
                    ...
                    @else
                        <button
                            class="btn-primary flex min-w-[70px] max-w-[400px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-9 px-3 text-xs font-semibold leading-normal tracking-[0.015em] shadow-sm">
                            <a href="{{ route('login') }}"
                                class="inline-block px-3 py-1 dark:text-[#EDEDEC] text-[#1b1b18] border border-transparent rounded-sm text-xs leading-normal">
                                Log in
                            </a>
                        </button>
                        ...
                        @if (Route::has('register'))
                            <button
                                ...
                                <a href="{{ route('register') }}"
                                    class="inline-block px-3 py-1 dark:text-[#EDEDEC] border-[#19140035] rounded-sm text-xs leading-normal">
                                    Register
                                </a>
                            </button>
                        @endif
